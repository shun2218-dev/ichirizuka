...
**アプリの依存を増やさない。** `dependencies` は `next` / `react` / `react-dom` のみを保つ。DB もチャートライブラリも状態管理ライブラリも使わない。グラフは SVG を直接書き、集計は素の TypeScript で行う。

**`scripts/` は別扱い。** オフラインで動く取り込み道具は、標準機能で書けないもの（バイナリ形式のデコードなど）に限り devDependency を足してよい。

迷ったら次の 3 つを問う。**すべて No なら足してよい。**

1. そのコードはブラウザに届くか
2. リクエストごとに動くか
3. 壊れたときダッシュボードが落ちるか

| | ブラウザに届く | リクエスト毎 | 落ちると死ぬ | 判定 |
| --- | --- | --- | --- | --- |
| チャートライブラリ | ○ | ○ | ○ | 不可 |
| DB | – | ○ | ○ | 不可 |
| FIT パーサ（`scripts/`） | – | – | – | 可 |

このルールが避けたいのは「アプリの構造を外部に決められること」であって、依存の数そのものではない。チャートライブラリは表示の書き方を、DB はデータの持ち方を縛る。一方、バイナリ形式のデコーダは構造を縛らない — ワークアウト 1 本につき 1 回オフラインで動き、数値を吐いたら終わる。**ここで自作を選ぶと、ルールが本来避けたかった「保守すべきコードが増える」を自分で招くことになる。**

依存を足すときは Issue に理由を残す。
